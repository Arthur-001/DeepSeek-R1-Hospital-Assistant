...
QUICK_TEST_CONFIG = {
    ...
    "test_size": 2,           # Number of examples to test
    ...
    "results_file": os.path.join("..", "evaluationResults", "quick_test_results_{timestamp}.json")  # Different results file with timestamp
}

# Model Paths
MODEL_PATHS = {
    "base_model": "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B",  # Base model path
    "fine_tuned_model": os.path.join("..", "Fine-Tuned Models", "finetuned_model_epochs_0.1_20250425_201530")  # Fine-tuned model path
}
...
EVAL_CONFIG = {
    "dataset_path": os.path.join("..", "data-manipulation", "dataset.json"),
    ...
    "results_file": os.path.join("..", "evaluationResults", "evaluation_results_{timestamp}.json"),  # Path to save results with timestamp
    "bertscore_model": "microsoft/deberta-xlarge-mnli",  # BERTScore model
    "compute_bertscore": False,  # Whether to compute BERTScore (can be slow)
    "compute_meteor": False,     # Whether to compute METEOR score
...
def get_results_filename(base_filename, quick_test=False):
    ...
    if quick_test:
        return os.path.abspath(os.path.join(os.path.dirname(__file__), QUICK_TEST_CONFIG["results_file"].format(timestamp=timestamp)))
    return os.path.abspath(os.path.join(os.path.dirname(__file__), EVAL_CONFIG["results_file"].format(timestamp=timestamp)))
...
def load_model_and_tokenizer():
    ...
    model_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), MODEL_PATHS["fine_tuned_model"]))
...
def load_dataset(quick_test=False):
    ...
    # Convert relative path to absolute path
    dataset_path = os.path.abspath(os.path.join(os.path.dirname(__file__), EVAL_CONFIG["dataset_path"]))
    ...
    try:
        ...
        raise ValueError(f"Error parsing JSON file at {dataset_path}: {str(e)}")
    except Exception as e:
        raise Exception(f"Error loading dataset from {dataset_path}: {str(e)}")
    
    if not dataset:
        raise ValueError(f"Dataset is empty at {dataset_path}")
...
def load_metrics(quick_test=False):
    ...
    metrics = {}
    # Use quick test metrics if in quick test mode, otherwise use all metrics
...
def compute_metrics(metrics, prediction, reference, quick_test=False):
    ...
    # Compute accuracy (only in full evaluation modes)
    if not quick_test:
        scores["accuracy"] = compute_accuracy(prediction, reference)
    
    # Compute F1 score (only in full evaluation modes)
    if not quick_test:
        scores["f1_score"] = compute_f1_score(prediction, reference)
    
    # Compute ROUGE scores (always compute as it's fast)
    if "rouge" in metrics:
        try:
    ...
    # Compute BLEU score (only in full evaluation modes)
    if not quick_test and "sacrebleu" in metrics:
        try:
    ...
    # Compute BERTScore (only in full evaluation modes)
    if not quick_test and "bertscore" in EVAL_CONFIG["metrics"]:
        try:
            P, R, F1 = bert_score(
                [prediction],
                [reference],
    ...
    # Compute METEOR score (only in full evaluation modes)
    if not quick_test and "meteor" in EVAL_CONFIG["metrics"]:
        try:
    ...
    # Compute exact match (always compute as it's fast)
    if "exact_match" in (QUICK_TEST_CONFIG["metrics"] if quick_test else EVAL_CONFIG["metrics"]):
